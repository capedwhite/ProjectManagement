    ...
    <main className="flex min-h-screen items-center justify-center bg-[#f8fafb] p-6">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 border border-slate-200/90">
        ...
        <div className="mb-6">
          ...
          <p className="text-xs text-slate-500 mt-1">Get started with your ProjectHub workspace</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-700">Full name</label>
            <input
              type="text"
              name="name"
              value={form.name}
              onChange={handleChange}
              className={`w-full rounded-xl border bg-slate-50/50 px-3.5 py-2.5 text-sm outline-none transition focus:border-[#3ec170] focus:bg-white focus:ring-1 focus:ring-[#3ec170] ${fieldErrors.name ? "border-red-400" : "border-slate-300"}`}
